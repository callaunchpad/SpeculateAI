...
def run_xgboost_model(data, window, n_estim, maxDepth, minSamplesSplit, minSamplesLeaf, n_job):
    ...
    return test_accuracy_percentage, train_labels, train_predicted_labels, test_labels, test_predicted_labels, train_accuracy_percentage, model
...
def main():
    ...
    def many_plots():
        ...
        def different_rf_windows_test():
            ...
            for j in range(iterations):
                ...
                for i in range(5, window_range, 5):
                    y.append(run_xgboost_model(data, i, n_estim, max_dep, min_split, min_leaf, n_job)[0])
                    x.append(i)
        ...
        def average_randomforest_test():
            ...
            for i in range(2, window_range):
                ...
                # x_labels.append(i)
                for j in range(10):
                    accuracy.append(run_xgboost_model(data, i, n_estim, max_dep, min_split, min_leaf, n_job)[0])
        ...
        def auc_nestimators():
            ...
            for estimator in n_estimators:
                acc, y_train, train_pred, y_test, y_pred, train_acc = run_xgboost_model(data, 5, estimator, max_dep, min_split,
                                                                                   min_leaf, -1)
        ...
        def max_depth():
            ...
            for max_depth in max_depths:
                acc, y_train, train_pred, y_test, y_pred, train_acc = run_xgboost_model(data, 5, n_estim, max_depth, min_split,
                                                                                   min_leaf, -1)
                false_positive_rate, true_positive_rate, thresholds = roc_curve(y_train, train_pred)
                roc_auc = auc(false_positive_rate, true_positive_rate)
                train_results.append(roc_auc)
        ...
        def min_sample_split():
            ...
            for min_samples_split in min_samples_splits:
                acc, y_train, train_pred, y_test, y_pred, train_acc = run_xgboost_model(data, 5, n_estim, max_dep, min_samples_split,
                                                                                   min_leaf, -1)
                false_positive_rate, true_positive_rate, thresholds = roc_curve(y_train, train_pred)
                roc_auc = auc(false_positive_rate, true_positive_rate)
                train_results.append(roc_auc)
        ...
        def min_sample_leaf():
            ...
            for min_samples_leaf in min_samples_leafs:
                acc, y_train, train_pred, y_test, y_pred, train_acc = run_xgboost_model(data, 5, n_estim, max_dep, min_split,
                                                                                   min_samples_leaf, -1)
                false_positive_rate, true_positive_rate, thresholds = roc_curve(y_train, train_pred)
                roc_auc = auc(false_positive_rate, true_positive_rate)
                train_results.append(roc_auc)
        ...
        def different_windows_train():
            ...
            for j in range(iterations):
                ...
                average_accuracy = None
                for i in range(5, window_range, 5):
                    y.append(run_xgboost_model(data, i, n_estim, max_dep, min_split, min_leaf, n_job)[5])
                    x.append(i)
        ...
        def average_randomforest_train():
            ...
            for i in range(2, window_range):
                accuracy = []
                for j in range(10):
                    accuracy.append(run_xgboost_model(data,i, n_estim, max_dep, min_split, min_leaf, n_job)[5])
                temp = statistics.mean(accuracy)
        ...
        def test_bad():
            plt.figure()
            for i in range(50):
                temp = run_xgboost_model(data, 36, n_estim, max_dep, min_split, min_sample_leaf, n_job)
    ...
    def feature_importance_graph():
        ...
        min_leaf = 1
        n_job = None
        model = run_xgboost_model(37, n_estim, max_dep, min_split, min_leaf, n_job)[6]
